...
```
/parallel review the auth module for bugs, perf, and API design
/parallel check every service dir for stale deps, one agent per service
```

## How many

One agent per real part — the split sets the count, not a quota. A ten-module repo gets ten agents. If the user names or implies a count, that wins.

Only floor: under 3 parts, do it inline. Spawning an agent to read a file you could read is pure overhead.

If parts depend on each other, or the work needs loops, retries, or staged verification, use `Workflow` instead and say so in one line.
...
Bad splits: parts that all read the same file, or one sequence chopped up. When you don't know the shape, scout with one `Grep`/`ls` and fan out over the real list — that scout also tells you the count.
...
If agents write, assign disjoint paths. If they can't be disjoint, pass `isolation: "worktree"` and merge the diffs yourself — two agents on one file is a silent last-write-wins.
...
Output one merged answer ordered by what matters most, not one section per agent. Lead with the count and split axis in one line.
